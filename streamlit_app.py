...
def get_service() -> ApplicationService:
    database_path = os.environ.get("APPLICATION_DB", "app/data/applications.db")
    return ApplicationService(Repository(database_path))
...
with profile_tab:
    ...
    with st.form("profile"):
        ...
        if st.form_submit_button("Save profile", type="primary"):
            try:
                service.save_profile(first_name=first_name, last_name=last_name, email=email, phone=phone, location=location, summary=summary)
...
with jobs_tab:
    ...
    with st.form("job"):
        ...
        if st.form_submit_button("Add to review queue", type="primary"):
            try:
                service.add_job(url, company, title)
                st.success("Job added to your review queue.")
            except ValueError as error:
                st.error(str(error))
...
with review_tab:
    ...
    if not pending:
        st.info("Your review queue is empty.")
    for item in pending:
        with st.expander(f'{item["title"] or "Untitled role"} · {item["company"] or "Unknown company"}'):
            ...
            if save.button("Save review", key=f'save-{item["id"]}'):
                service.move_application(item["id"], status, notes)
                st.rerun()
            if remove.button("Delete", key=f'delete-{item["id"]}'):
                service.delete_application(item["id"])
                st.rerun()
with history_tab:
    if applications:
